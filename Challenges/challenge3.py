...
def pitch_estimate(pitch2, dt, alpha):
    theta = int(imu.pitch())
    pitch_dot = imu.get_gy()
    pitch = alpha*(pitch2 + pitch_dot*(dt/1000000)) + (1-alpha) * theta
    return (pitch)
...
while not trigger():  # wait to tune Kp
    pyb.delay(1)
    K_p = pot.read() * (scale*5) / 4095  # use pot to set Kp
...
while not trigger():  # wait to tune Ki
    pyb.delay(1)
    K_i = pot.read() * (scale*50) / 4095  # use pot to set Ki
...
try:  # Try to handle exception
    ...
    while True:  # infinite loop
        ...
        if (dt > 5000):  # loop period is 5 msec or 200Hz
            # esitmate pitch angle and pit dot
            pitch = int(imu.pitch())
            pitch_dot = imu.get_gy()
            pitch = pitch_estimate(pitch, dt, alpha)
            #pitch, pitch_dot =
            # update tic1
            tic1 = pyb.micros()
            # do PID control
